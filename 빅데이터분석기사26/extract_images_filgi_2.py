...
def extract_images_from_pdf(pdf_path, output_dir):
    ...
    for i, page in enumerate(reader.pages):
        ...
        try:
            ...
            # If we find images, save them.
            # We try to name them page_XXX.ext
            # If multiple, page_XXX_img_Y.ext
            for j, image in enumerate(images):
                # image.name is usually the filename inside the PDF
                name = image.name
...
if __name__ == "__main__":
    ...
    file_path = os.path.join(base_path, "빅데이터분석기사26_필기2/빅데이터분석기사26_필기2.pdf")
    output_path = os.path.join(base_path, "빅데이터분석기사26_필기2/img")
