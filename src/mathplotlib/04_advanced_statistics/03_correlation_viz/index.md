---
layout: mathplotlib
title: "5.4.3 상관관계 폭격기: Heatmap과 Pairplot"
description: "데이터 분석에서 가장 짜릿한 순간은 \"A가 오르면 B도 오르더라!\" 같은 숨겨진 데이터 간의 연관성(상관관계)을 밝혀내는 순간입니다. 열(피처)이 수십 개가 넘어가는 실무 데이터셋에서 이를 일일이 산점도로 그리는 것은 불가능하므로, 우리에겐 Heatmap과 Pairplot..."
---
...
> - [📥 correlation_viz_practice.ipynb 파일 다운로드](./correlation_viz_practice.ipynb) (클릭 또는 마우스 우클릭 후 '다른 이름으로 링크 저장')

데이터 분석에서 가장 짜릿한 순간은 "A가 오르면 B도 오르더라!" 같은 숨겨진 데이터 간의 연관성(상관관계)을 밝혀내는 순간입니다. 열(피처)이 수십 개가 넘어가는 실무 데이터셋에서 이를 일일이 산점도로 그리는 것은 불가능하므로, 우리에겐 `Heatmap`과 `Pairplot`이라는 궁극의 무기가 필요합니다.

## [실습 1] 상관계수와 히트맵 (Heatmap)의 마법
...
## [실습 2] 융단 폭격: 산점도 행렬 `sns.pairplot()`
