---
layout: mathplotlib
title: "5.3.3 그래프 속성과 스타일링"
description: "Matplotlib의 핵심 속성(Attribute)들을 이해하고, 이를 조합하여 나만의 멋진 그래프를 디자인할 수 있습니다."
---
...
> 💾 **[실습 파일 다운로드]**
> 본 강의의 스타일링 실습 코드를 직접 실행해 볼 수 있는 주피터 노트북 파일입니다. 
> - [📥 styling_practice.ipynb 파일 다운로드](./styling_practice.ipynb) (클릭 또는 마우스 우클릭 후 '다른 이름으로 링크 저장')
...
**[코드 설명]**
- `c='b', ls='-', marker='o', lw=2`: 용사의 선을 파란색(`b`), 실선(`-`), 동그라미 마커(`o`), 굵기 2로 꾸밉니다.
- `c='r', ls='-.', marker='x', lw=2`: 몬스터의 선을 빨간색(`r`), 점+실선 혼합인 쇄선(`-.`), X 모양 마커(`x`), 굵기 2로 꾸밉니다.
- `fontsize=15`: 제목과 라벨의 폰트 크기를 키워서 가시성을 높입니다.
- `plt.grid(...)`: `axis='y'`를 주어 가로선 그리드만 표시하며, `alpha=0.5`로 반투명하게 만들어 선 그래프를 돋보이게 합니다.
