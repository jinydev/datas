...
### 1단계: `df.head()` - 서류 봉투 살짝 열어보기
수백만 줄의 데이터를 한 번에 화면에 띄우면 컴퓨터가 멈춰버립니다. `.head()`는 상위 5개의 줄만 살짝 뽑아서 이 표가 어떤 컬럼(속성)들로 이루어져 있는지 파악하는 첫 번째 관문입니다.
