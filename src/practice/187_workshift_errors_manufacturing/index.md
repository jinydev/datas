---
layout: practice
title: "187. 근무조 교대 근무 패턴과 작업 실수 상관 실습"
permalink: /practice/187_workshift_errors_manufacturing/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 근무조 교대 근무 패턴과 작업 실수 상관를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 median 대치 (\`fillna\`):** 장비 예지 센서 일시 노이즈 또는 PLC 데이터 전송 불량으로 발생한 불량발생건수 변수의 빈칸을 안전하게 채웁니다.
...
memory usage: 47.0 KB
> 
>    ShiftID  WeeklyOvernightShifts  TeamSize  DefectiveItemsProduced  InspectAccuracy_Percent  IncidentReported
0  1870001                  134.2     124.5                   102.6                     51.8                 1
1  1870002                   45.2     125.4                   140.9                     60.0                 1
2  1870003                  104.9     120.6                    80.2                     49.4                 1
3  1870004                   89.7     103.2                   105.0                     58.6                 0
4  1870005                  123.0     107.8                    99.1                     46.9                 0
> ```
...
# 2. 결측치 전처리 및 대치 수행
median_val = df['DefectiveItemsProduced'].median().round(1)
df['DefectiveItemsProduced'] = df['DefectiveItemsProduced'].fillna(median_val)
print(df.isnull().sum())
```
...
* **중앙값 대입 적용의 비즈니스 및 통계적 근거:** 불량발생건수 정보는 긴 꼬리 분포나 일부 특이 이상치에 의해 평균이 한쪽으로 치우치기 쉬우므로 통계적 안정성이 강한 중앙값으로 결측을 대치합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='DefectiveItemsProduced', kde=True, color='teal')
plt.title('근무조 교대 근무 패턴과 작업 실수 상관 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='WeeklyOvernightShifts', y='DefectiveItemsProduced', hue='IncidentReported', palette='coolwarm', alpha=0.8)
plt.title('WeeklyOvernightShifts와 DefectiveItemsProduced 상관성 및 IncidentReported 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 근무조 교대 근무 패턴과 작업 실수 상관를 판독하기 위한 의사결정 프레임워크를 제공합니다.
