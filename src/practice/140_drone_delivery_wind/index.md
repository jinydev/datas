---
layout: practice
title: "140. 물류 배송 드론 풍속 저항 및 비행 한계 실습"
permalink: /practice/140_drone_delivery_wind/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 물류 배송 드론 풍속 저항 및 비행 한계를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 mean 대치 (\`fillna\`):** 이송 센서 통신 일시 음영 대역 또는 수송 GPS 튕김으로 발생한 잔여비행거리 변수의 빈칸을 안전하게 채웁니다.
...
memory usage: 47.0 KB
> 
>    FlightID  WindSpeedKmh  PayloadWeight_Kg  FlightRangeRemaining_Km  VoltageDrop_Volts  MissionAborted
0   1400001           9.7              81.3                      7.7               17.0            80.3
1   1400002          13.5             100.4                     17.6               52.0           136.1
2   1400003          12.9             141.8                     20.5               53.0           102.8
3   1400004           7.9             112.2                      4.2               23.0            99.6
4   1400005          11.8             138.7                      1.7               43.0            88.3
> ```
...
# 2. 결측치 전처리 및 대치 수행
mean_val = df['FlightRangeRemaining_Km'].mean().round(1)
df['FlightRangeRemaining_Km'] = df['FlightRangeRemaining_Km'].fillna(mean_val)
print(df.isnull().sum())
```
...
* **평균 대입 적용의 비즈니스 및 통계적 근거:** 잔여비행거리 지표는 평시 상태에서 정규분포 중심에 수렴하므로, 누락된 값을 피실험자 및 이용자 평균값으로 메워 왜곡을 최소화합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='FlightRangeRemaining_Km', kde=True, color='teal')
plt.title('물류 배송 드론 풍속 저항 및 비행 한계 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='PayloadWeight_Kg', y='FlightRangeRemaining_Km', hue='MissionAborted', palette='coolwarm', alpha=0.8)
plt.title('PayloadWeight_Kg와 FlightRangeRemaining_Km 상관성 및 MissionAborted 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 물류 배송 드론 풍속 저항 및 비행 한계를 판독하기 위한 의사결정 프레임워크를 제공합니다.
