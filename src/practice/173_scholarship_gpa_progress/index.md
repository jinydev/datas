---
layout: practice
title: "173. 대학 장학금 지원 예산 분배 효율성 실습"
permalink: /practice/173_scholarship_gpa_progress/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 대학 장학금 지원 예산 분배 효율성를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 median 대치 (\`fillna\`):** 모바일 학습 앱 로그 전송 끊김 또는 설문 불성실 응답으로 발생한 주간학습시간 변수의 빈칸을 안전하게 채웁니다.
...
memory usage: 47.0 KB
> 
>    StudentID  ScholarshipAmount_K  PriorGPA  PostGPA  StudyHoursWeekly  GPASignificantlyImproved
0    1730001                 80.5      57.4    115.7              27.7                         1
1    1730002                 64.4      74.9     83.2              18.9                         1
2    1730003                126.3      51.7    101.4              38.3                         1
3    1730004                 88.8      72.1    102.5              24.8                         1
4    1730005                102.7     127.2     55.9              39.8                         1
> ```
...
# 2. 결측치 전처리 및 대치 수행
median_val = df['StudyHoursWeekly'].median().round(1)
df['StudyHoursWeekly'] = df['StudyHoursWeekly'].fillna(median_val)
print(df.isnull().sum())
```
...
* **중앙값 대입 적용의 비즈니스 및 통계적 근거:** 주간학습시간 정보는 긴 꼬리 분포나 일부 특이 이상치에 의해 평균이 한쪽으로 치우치기 쉬우므로 통계적 안정성이 강한 중앙값으로 결측을 대치합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='PostGPA', kde=True, color='teal')
plt.title('대학 장학금 지원 예산 분배 효율성 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='ScholarshipAmount_K', y='PostGPA', hue='GPASignificantlyImproved', palette='coolwarm', alpha=0.8)
plt.title('ScholarshipAmount_K와 PostGPA 상관성 및 GPASignificantlyImproved 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 대학 장학금 지원 예산 분배 효율성를 판독하기 위한 의사결정 프레임워크를 제공합니다.
