---
layout: practice
title: "179. 온라인 튜터 매칭 만족도 및 재수강률 실습"
permalink: /practice/179_tutor_matching_success/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 온라인 튜터 매칭 만족도 및 재수강률를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 mean 대치 (\`fillna\`):** 모바일 학습 앱 로그 전송 끊김 또는 설문 불성실 응답으로 발생한 학생만족점수 변수의 빈칸을 안전하게 채웁니다.
...
memory usage: 47.0 KB
> 
>    MatchID  TutorExpYears  ClassFeeHourly  StudentSatisfactionScore  LessonsCompleted  RenewedSubscription
0  1790001           14.3            94.0                       6.3             114.0                115.9
1  1790002            5.1           138.4                       7.1             102.6                 79.1
2  1790003           13.2           127.7                       7.9              87.4                106.7
3  1790004           12.0            63.9                       4.9              82.2                100.6
4  1790005            6.2           156.9                       5.5             106.8                 73.0
> ```
...
# 2. 결측치 전처리 및 대치 수행
mean_val = df['StudentSatisfactionScore'].mean().round(1)
df['StudentSatisfactionScore'] = df['StudentSatisfactionScore'].fillna(mean_val)
print(df.isnull().sum())
```
...
* **평균 대입 적용의 비즈니스 및 통계적 근거:** 학생만족점수 지표는 평시 상태에서 정규분포 중심에 수렴하므로, 누락된 값을 피실험자 및 이용자 평균값으로 메워 왜곡을 최소화합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='LessonsCompleted', kde=True, color='teal')
plt.title('온라인 튜터 매칭 만족도 및 재수강률 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='TutorExpYears', y='LessonsCompleted', hue='RenewedSubscription', palette='coolwarm', alpha=0.8)
plt.title('TutorExpYears와 LessonsCompleted 상관성 및 RenewedSubscription 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 온라인 튜터 매칭 만족도 및 재수강률를 판독하기 위한 의사결정 프레임워크를 제공합니다.
