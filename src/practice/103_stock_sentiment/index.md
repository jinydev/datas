---
layout: practice
title: "103. 주식 시장 소셜 미디어 감성 지수 실습"
permalink: /practice/103_stock_sentiment/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 주식 시장 소셜 미디어 감성 지수를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 median 대치 (\`fillna\`):** 고객 설문 응답 누락 또는 신용 기관 연동 오류으로 발생한 기사량 변수의 빈칸을 안전하게 채웁니다.
...
memory usage: 47.0 KB
> 
>    ArticleID  PolarityScore  Volume_K  PriceGap_Percent  Institutional_Ratio  MarketImpactScore
0    1030001            5.3      77.7              72.4                 68.7                7.5
1    1030002            6.8     111.0              25.7                 77.1                6.6
2    1030003            7.8      69.1              47.0                 23.3                8.4
3    1030004            6.6     103.2              60.7                 42.8                7.5
4    1030005            5.6      97.3              47.8                 38.8                7.5
> ```
...
# 2. 결측치 전처리 및 대치 수행
median_val = df['Volume_K'].median().round(1)
df['Volume_K'] = df['Volume_K'].fillna(median_val)
print(df.isnull().sum())
```
...
* **중앙값 대입 적용의 비즈니스 및 통계적 근거:** 기사량 정보는 긴 꼬리 분포나 일부 특이 이상치에 의해 평균이 한쪽으로 치우치기 쉬우므로 통계적 안정성이 강한 중앙값으로 결측을 대치합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='MarketImpactScore', kde=True, color='teal')
plt.title('주식 시장 소셜 미디어 감성 지수 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='PolarityScore', y='MarketImpactScore', hue='Institutional_Ratio', palette='coolwarm', alpha=0.8)
plt.title('PolarityScore와 MarketImpactScore 상관성 및 Institutional_Ratio 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 주식 시장 소셜 미디어 감성 지수를 판독하기 위한 의사결정 프레임워크를 제공합니다.
