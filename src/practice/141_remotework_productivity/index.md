---
layout: practice
title: "141. 원격 근무 스트레스 및 성과 상관성 실습"
permalink: /practice/141_remotework_productivity/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 원격 근무 스트레스 및 성과 상관성를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 mean 대치 (\`fillna\`):** 설문 평가 응답 거부 또는 사내 HR 데이터 연동 누락으로 발생한 태스크완료율 변수의 빈칸을 안전하게 채웁니다.
...
memory usage: 47.0 KB
> 
>    EmployeeID  MeetingHours_Weekly  RemoteDaysCount  SelfReportedStress  TaskCompletionRate_Percent  PerformanceGrade
0     1410001                 41.7            119.1                83.9                        17.0              91.2
1     1410002                 25.9            111.8               113.3                        70.4              84.6
2     1410003                  6.7            107.1               130.4                        53.5              76.2
3     1410004                 21.9            123.6               112.3                        31.4              57.1
4     1410005                 33.6            159.6                99.6                         NaN              94.9
> ```
...
# 2. 결측치 전처리 및 대치 수행
mean_val = df['TaskCompletionRate_Percent'].mean().round(1)
df['TaskCompletionRate_Percent'] = df['TaskCompletionRate_Percent'].fillna(mean_val)
print(df.isnull().sum())
```
...
* **평균 대입 적용의 비즈니스 및 통계적 근거:** 태스크완료율 지표는 평시 상태에서 정규분포 중심에 수렴하므로, 누락된 값을 피실험자 및 이용자 평균값으로 메워 왜곡을 최소화합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='TaskCompletionRate_Percent', kde=True, color='teal')
plt.title('원격 근무 스트레스 및 성과 상관성 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='MeetingHours_Weekly', y='TaskCompletionRate_Percent', hue='SelfReportedStress', palette='coolwarm', alpha=0.8)
plt.title('MeetingHours_Weekly와 TaskCompletionRate_Percent 상관성 및 SelfReportedStress 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 원격 근무 스트레스 및 성과 상관성를 판독하기 위한 의사결정 프레임워크를 제공합니다.
