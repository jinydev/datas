---
layout: practice
title: "129. 헌혈 희망자 자격 검격 및 혈액 보존 실습"
permalink: /practice/129_blood_donor_supply/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 헌혈 희망자 자격 검격 및 혈액 보존를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 mean 대치 (\`fillna\`):** 모바일 생체 검진 기기 전원 방전 또는 환자 기재 미비으로 발생한 헤모글로빈지수 변수의 빈칸을 안전하게 채웁니다.
...
memory usage: 47.0 KB
> 
>    DonorID  Hemoglobin_Level  BloodPressureSystolic  DeferralHistory  DonorAge  DonationSuccess
0  1290001              59.3                   2.18            130.6      38.0                1
1  1290002             112.4                   0.92             71.1      19.0                1
2  1290003              80.9                   1.53            141.3      18.0                1
3  1290004             104.8                   1.52             96.4      53.0                1
4  1290005             100.4                   1.66            106.9      35.0                0
> ```
...
# 2. 결측치 전처리 및 대치 수행
mean_val = df['Hemoglobin_Level'].mean().round(1)
df['Hemoglobin_Level'] = df['Hemoglobin_Level'].fillna(mean_val)
print(df.isnull().sum())
```
...
* **평균 대입 적용의 비즈니스 및 통계적 근거:** 헤모글로빈지수 지표는 평시 상태에서 정규분포 중심에 수렴하므로, 누락된 값을 피실험자 및 이용자 평균값으로 메워 왜곡을 최소화합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='Hemoglobin_Level', kde=True, color='teal')
plt.title('헌혈 희망자 자격 검격 및 혈액 보존 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='DonorAge', y='Hemoglobin_Level', hue='DonationSuccess', palette='coolwarm', alpha=0.8)
plt.title('DonorAge와 Hemoglobin_Level 상관성 및 DonationSuccess 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 헌혈 희망자 자격 검격 및 혈액 보존를 판독하기 위한 의사결정 프레임워크를 제공합니다.
