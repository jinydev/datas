---
layout: practice
title: "136. 도시 지하철 출퇴근 시간대 혼잡도 실습"
permalink: /practice/136_subway_rushhour_congestion/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 도시 지하철 출퇴근 시간대 혼잡도를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 mean 대치 (\`fillna\`):** 이송 센서 통신 일시 음영 대역 또는 수송 GPS 튕김으로 발생한 혼잡도지표 변수의 빈칸을 안전하게 채웁니다.
...
> 
>    StationID  BoardingPassengerCount  IntervalTimeSeconds LineStatus  CongestionScore  TrainDelayMinutes
0    1360001                    96.1                  8.9     Type_B              7.1               17.8
1    1360002                    96.1                 25.6     Type_A              7.2                8.0
2    1360003                    87.5                  2.6     Type_A              6.1                3.5
3    1360004                    89.7                  2.8     Type_B              3.4                4.1
4    1360005                    82.2                 17.1     Type_B              9.0                6.1
> ```
...
# 2. 결측치 전처리 및 대치 수행
mean_val = df['CongestionScore'].mean().round(1)
df['CongestionScore'] = df['CongestionScore'].fillna(mean_val)
print(df.isnull().sum())
```
...
* **평균 대입 적용의 비즈니스 및 통계적 근거:** 혼잡도지표 지표는 평시 상태에서 정규분포 중심에 수렴하므로, 누락된 값을 피실험자 및 이용자 평균값으로 메워 왜곡을 최소화합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='CongestionScore', kde=True, color='teal')
plt.title('도시 지하철 출퇴근 시간대 혼잡도 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='BoardingPassengerCount', y='CongestionScore', hue='LineStatus', palette='coolwarm', alpha=0.8)
plt.title('BoardingPassengerCount와 CongestionScore 상관성 및 LineStatus 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 도시 지하철 출퇴근 시간대 혼잡도를 판독하기 위한 의사결정 프레임워크를 제공합니다.
