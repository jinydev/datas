---
layout: practice
title: "169. 스트리머 실시간 방송 채팅 트래픽 스파이크 실습"
permalink: /practice/169_livestream_chat_spikes/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 스트리머 실시간 방송 채팅 트래픽 스파이크를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 mean 대치 (\`fillna\`):** 실시간 서버 API 트래픽 폭주 및 로그 저장 지연으로 발생한 분당채팅수 변수의 빈칸을 안전하게 채웁니다.
...
memory usage: 47.0 KB
> 
>    StreamID  ConcurrentViewers  ChatRatePerMinute  ToxicityRatio_Percent  NetworkLatency_ms  ServerLoadLevel
0   1690001               5.28               64.4                   51.6              111.0                1
1   1690002               4.83               47.9                   64.5               73.4                1
2   1690003               3.81               55.4                   54.0              123.7                1
3   1690004               4.78               37.7                   30.1               91.0                1
4   1690005               5.39               31.6                   33.3              133.5                1
> ```
...
# 2. 결측치 전처리 및 대치 수행
mean_val = df['ChatRatePerMinute'].mean().round(1)
df['ChatRatePerMinute'] = df['ChatRatePerMinute'].fillna(mean_val)
print(df.isnull().sum())
```
...
* **평균 대입 적용의 비즈니스 및 통계적 근거:** 분당채팅수 지표는 평시 상태에서 정규분포 중심에 수렴하므로, 누락된 값을 피실험자 및 이용자 평균값으로 메워 왜곡을 최소화합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='ChatRatePerMinute', kde=True, color='teal')
plt.title('스트리머 실시간 방송 채팅 트래픽 스파이크 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='ConcurrentViewers', y='ChatRatePerMinute', hue='ServerLoadLevel', palette='coolwarm', alpha=0.8)
plt.title('ConcurrentViewers와 ChatRatePerMinute 상관성 및 ServerLoadLevel 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 스트리머 실시간 방송 채팅 트래픽 스파이크를 판독하기 위한 의사결정 프레임워크를 제공합니다.
