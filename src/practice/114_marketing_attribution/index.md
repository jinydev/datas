---
layout: practice
title: "114. 디지털 마케팅 채널 기여도 (Attribution) 실습"
permalink: /practice/114_marketing_attribution/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 디지털 마케팅 채널 기여도 (Attribution)를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 median 대치 (\`fillna\`):** 구매 전환 기록 누락 또는 웹로그 트래킹 지연으로 발생한 광고지출액 변수의 빈칸을 안전하게 채웁니다.
...
memory usage: 47.0 KB
> 
>    CampaignID  AdSpend_K  SocialMediaImpressions  SearchClicks  DirectVisits  ConversionRevenue
0     1140001       74.6                   131.0         103.6          86.9            60062.0
1     1140002       91.0                   116.0          89.5          94.9            67338.0
2     1140003       61.8                    92.8          64.2         122.4            79283.0
3     1140004      100.4                   134.4         124.3         101.6            78929.0
4     1140005       76.3                    67.4         131.4         113.9            69391.0
> ```
...
# 2. 결측치 전처리 및 대치 수행
median_val = df['AdSpend_K'].median().round(1)
df['AdSpend_K'] = df['AdSpend_K'].fillna(median_val)
print(df.isnull().sum())
```
...
* **중앙값 대입 적용의 비즈니스 및 통계적 근거:** 광고지출액 정보는 긴 꼬리 분포나 일부 특이 이상치에 의해 평균이 한쪽으로 치우치기 쉬우므로 통계적 안정성이 강한 중앙값으로 결측을 대치합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='ConversionRevenue', kde=True, color='teal')
plt.title('디지털 마케팅 채널 기여도 (Attribution) 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='SocialMediaImpressions', y='ConversionRevenue', hue='SearchClicks', palette='coolwarm', alpha=0.8)
plt.title('SocialMediaImpressions와 ConversionRevenue 상관성 및 SearchClicks 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 디지털 마케팅 채널 기여도 (Attribution)를 판독하기 위한 의사결정 프레임워크를 제공합니다.
