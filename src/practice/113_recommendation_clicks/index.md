---
layout: practice
title: "113. 하이브리드 협업 필터링 추천 시스템 실습"
permalink: /practice/113_recommendation_clicks/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 하이브리드 협업 필터링 추천 시스템를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 mean 대치 (\`fillna\`):** 구매 전환 기록 누락 또는 웹로그 트래킹 지연으로 발생한 매칭점수 변수의 빈칸을 안전하게 채웁니다.
...
> 
>    RecommendID  MatchingScore  PriceScore  UserAge ItemCategory  RecommendClicked
0      1130001            7.0         7.2     18.0       Type_B                 1
1      1130002            8.6         7.0     69.0       Type_C                 1
2      1130003            7.5         8.1     31.0       Type_B                 1
3      1130004            5.7         7.7     43.0       Type_B                 1
4      1130005            6.9         5.1     18.0       Type_A                 0
> ```
...
# 2. 결측치 전처리 및 대치 수행
mean_val = df['MatchingScore'].mean().round(1)
df['MatchingScore'] = df['MatchingScore'].fillna(mean_val)
print(df.isnull().sum())
```
...
* **평균 대입 적용의 비즈니스 및 통계적 근거:** 매칭점수 지표는 평시 상태에서 정규분포 중심에 수렴하므로, 누락된 값을 피실험자 및 이용자 평균값으로 메워 왜곡을 최소화합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='MatchingScore', kde=True, color='teal')
plt.title('하이브리드 협업 필터링 추천 시스템 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='MatchingScore', y='RecommendClicked', hue='ItemCategory', palette='coolwarm', alpha=0.8)
plt.title('MatchingScore와 RecommendClicked 상관성 및 ItemCategory 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 하이브리드 협업 필터링 추천 시스템를 판독하기 위한 의사결정 프레임워크를 제공합니다.
