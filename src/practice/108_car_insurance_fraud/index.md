---
layout: practice
title: "108. 자동차 사고 보험 청구 허위 사기 적발 실습"
permalink: /practice/108_car_insurance_fraud/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 자동차 사고 보험 청구 허위 사기 적발를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 mean 대치 (\`fillna\`):** 고객 설문 응답 누락 또는 신용 기관 연동 오류으로 발생한 사고 손상점수 변수의 빈칸을 안전하게 채웁니다.
...
# 2. 결측치 전처리 및 대치 수행
mean_val = df['DamageScore'].mean().round(1)
df['DamageScore'] = df['DamageScore'].fillna(mean_val)
print(df.isnull().sum())
```
...
* **평균 대입 적용의 비즈니스 및 통계적 근거:** 사고 손상점수 지표는 평시 상태에서 정규분포 중심에 수렴하므로, 누락된 값을 피실험자 및 이용자 평균값으로 메워 왜곡을 최소화합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='ClaimAmount_K', kde=True, color='teal')
plt.title('자동차 사고 보험 청구 허위 사기 적발 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='DamageScore', y='ClaimAmount_K', hue='IsFraud', palette='coolwarm', alpha=0.8)
plt.title('DamageScore와 ClaimAmount_K 상관성 및 IsFraud 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 자동차 사고 보험 청구 허위 사기 적발를 판독하기 위한 의사결정 프레임워크를 제공합니다.
