---
layout: practice
title: "118. 멤버십 포인트 적립 및 소진 행동 실습"
permalink: /practice/118_loyalty_points_redemption/
---
...
본 실습은 실제 비즈니스 및 학계에서 자주 마주하는 멤버십 포인트 적립 및 소진 행동를 주제로 다룹니다.
수집된 실제 통계 데이터셋을 바탕으로 Pandas 라이브러리를 통해 결측치를 과학적으로 정제하고, Seaborn 시각화를 통해 다중 요인의 입체적인 상관 경향을 진단합니다.

---

### 🛠️ 핵심 분석 실습 역량 (Core Skills)
* **결측치 median 대치 (\`fillna\`):** 구매 전환 기록 누락 또는 웹로그 트래킹 지연으로 발생한 적립포인트 변수의 빈칸을 안전하게 채웁니다.
...
> 
>    MemberID  PointsAccumulated  RedemptionIntervalMonths LoyaltyTier  TotalSpend_K  Redeemed
0   1180001              102.9                      58.0      Type_A          88.9     128.6
1   1180002              116.0                      28.0      Type_C          77.1     115.2
2   1180003              113.7                      37.0      Type_C          60.0     119.7
3   1180004              104.4                      33.0      Type_C          69.5     102.9
4   1180005              105.1                      13.0      Type_B          87.1      91.1
> ```
...
# 2. 결측치 전처리 및 대치 수행
median_val = df['PointsAccumulated'].median().round(1)
df['PointsAccumulated'] = df['PointsAccumulated'].fillna(median_val)
print(df.isnull().sum())
```
...
* **중앙값 대입 적용의 비즈니스 및 통계적 근거:** 적립포인트 정보는 긴 꼬리 분포나 일부 특이 이상치에 의해 평균이 한쪽으로 치우치기 쉬우므로 통계적 안정성이 강한 중앙값으로 결측을 대치합니다.
...
# 단변수 분석 그래프 생성
sns.histplot(data=df, x='PointsAccumulated', kde=True, color='teal')
plt.title('멤버십 포인트 적립 및 소진 행동 빈도 분포', fontsize=14, fontweight='bold')
plt.show()
```
...
# 다변수 분석 그래프 생성
sns.scatterplot(data=df, x='PointsAccumulated', y='TotalSpend_K', hue='Redeemed', palette='coolwarm', alpha=0.8)
plt.title('PointsAccumulated와 TotalSpend_K 상관성 및 Redeemed 대조', fontsize=14, fontweight='bold')
plt.show()
```
...
> 💡 **[분석 도메인 통계 지식 한눈에 보기]**
> 본 실습은 멤버십 포인트 적립 및 소진 행동를 판독하기 위한 의사결정 프레임워크를 제공합니다.
