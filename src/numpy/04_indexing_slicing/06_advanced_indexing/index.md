...
슬라이싱은 `2:5`처럼 연속된 덩어리만 자를 수 있지만, **팬시 인덱싱(Fancy Indexing)**은 원하는 원소의 번호를 담은 리스트 `[0, 2, 5]`를 던져주어 띄엄띄엄 떨어진 "즐겨찾기 목록" 데이터만 쏙쏙 끄집어내는 아주 직관적인 기술입니다.
...
수만 개의 데이터에 대고 "이 중에 홀수이거나 10 이상인 애들 기립해!"라는 **마스크(True/False)**를 뒤집어씌우는 것과 같습니다. 조건식 필터를 적용해 `True`인 원소들만 체로 걸러지듯 즉시 반환되는 파워풀한 데이터 전처리 기술입니다.
