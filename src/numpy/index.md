...
title: "넘파이 (Numpy) 기초"
permalink: /numpy/
description: "이 섹션에서는 데이터 과학과 수치 계산의 핵심 패키지인 Numpy(넘파이)에 대해 다룹니다."
---

# 넘파이 (Numpy) 핵심 가이드

이 섹션에서는 데이터 과학과 수치 계산의 핵심 패키지인 **Numpy(넘파이)**에 대해 다룹니다.
데이터 구조를 극적으로 단순화시키는 '방정식과 행렬'의 수학적 원리부터 시작하여, 다차원 배열(ndarray) 객체의 생성, 슬라이싱, 형태 변경, 브로드캐스팅, 그리고 다양한 수학 연산 및 배열 결합/분할 함수까지 깊이 있는 내용을 논리적 섹션으로 그룹화하여 학습합니다.

![Numpy Illustration](./img/numpy_illustration.png)
...
### [4.5 행렬 연산과 범용 함수](05_operations/)
Numpy 배열간의 기본 사칙연산과 다양한 통계 요약 정보를 한 방에 처리하는 범용 함수(ufunc)를 배웁니다.
* [4.5.1 배열과 스칼라와의 연산](05_operations/01_scalar_operations/)
* [4.5.2 배열과 배열의 연산](05_operations/02_array_operations/)
* [4.5.3 배열 함수와 범용 함수](05_operations/03_ufunc/)

### [4.6 브로드캐스팅 (Broadcasting)](06_brodcasting/)
형태가 다른 배열 간의 연산을 가능하게 해주는 마법 같은 차원 확대 및 매칭 기술인 브로드캐스팅을 훈련합니다.
* [4.6.1 브로드캐스팅 개요](06_brodcasting/01_broadcasting_intro/)
* [4.6.2 스칼라 또는 단일 값과 배열의 연산](06_brodcasting/02_scalar_broadcasting/)
* [4.6.3 2차원 배열의 브로드캐스팅](06_brodcasting/03_d_broadcasting/)
* [4.6.4 상수 np.newaxis 활용해 두 배열 차원 추가](06_brodcasting/04_newaxis/)

### [4.7 배열 슬라이싱 (Array Slicing)](07_slicing/)
초대용량 빅데이터에서 원하는 부분만 가장 빠르고 세련되게 추출해내는 조건 검색 및 슬라이싱(자르기) 기술을 훈련합니다.
* [4.7.1 1차원 배열의 첨자와 슬라이싱](07_slicing/01_d_slicing/)
* [4.7.2 2차원 배열의 첨자와 슬라이싱](07_slicing/02_d_slicing/)
* [4.7.3 3차원 배열의 첨자와 슬라이싱](07_slicing/03_d_slicing/)

### [4.8 배열 인덱싱 (Array Indexing)](08_indexing/)
데이터의 특정 위치를 정확히 찌르거나 조건에 맞는 요소를 논리적으로 필터링하는 배열 색인(Indexing) 기술을 배웁니다.
* [4.8.1 1차원 배열의 색인](08_indexing/01_d_indexing/)
* [4.8.2 2차원 배열의 색인](08_indexing/02_d_indexing/)
* [4.8.3 다양한 슬라이싱과 팬시/불리언 배열 색인](08_indexing/03_advanced_indexing/)

### [4.9 배열 형태 변환과 메모리 시점](09_shape_memory/)
배열의 생김새를 뒤집거나, 원본을 보존한 채 참조 복사(View)만으로 새로운 각도에서 다차원 데이터를 바라보는 고급 메모리 관리 지식을 배웁니다.
* [4.9.1 배열 평탄화 (ravel)](09_shape_memory/01_reshape_ravel/)
* [4.9.2 대칭 반전과 축 교환 (Transpose & Swapaxes)](09_shape_memory/02_transpose_swapaxes/)
* [4.9.3 메모리 뷰 (View)와 얕은 복사](09_shape_memory/03_view/)
* [4.9.4 독립적 복사본 (Copy)과 base 추적](09_shape_memory/04_copy/)

### [4.10 배열 결합과 분할](10_concat_split/)
작은 데이터 블록 조각들을 가로/세로 축을 기준으로 결합해 병합 데이터셋을 조립하거나 쪼개 분할하는 방법을 배웁니다.
* [4.10.1 배열 결합 개요](10_concat_split/01_array_concat_intro/)
* [4.10.2 numpy.vstack()](10_concat_split/02_vstack/)
* [4.10.3 numpy.hstack()](10_concat_split/03_hstack/)
* [4.10.4 배열 결합 연산 column_stack()과 row_stack()](10_concat_split/04_column_row_stack/)
* [4.10.5 배열 결합 연산 concatenate()와 stack()](10_concat_split/05_concatenate_stack/)
* [4.10.6 column_stack()과 hstack()](10_concat_split/06_column_stack_vs_hstack/)
* [4.10.7 row_stack()과 vstack()](10_concat_split/07_row_stack_vs_vstack/)
* [4.10.8 np.c_[]](10_concat_split/08_npc_column_stack/)
* [4.10.9 np.r_[]](10_concat_split/09_npr/)
* [4.10.10 1차원 배열, 벡터의 다양한 결합 방법](10_concat_split/10_d_concat_methods/)
* [4.10.11 1차원 배열의 다양한 결합 방법 정리](10_concat_split/11_d_concat_summary/)
* [4.10.12 유용한 함수 split()](10_concat_split/12_split/)
* [4.10.13 분할 함수 hsplit()과 vsplit()](10_concat_split/13_hsplit_vsplit/)
* [4.10.14 분할 함수 array_split()](10_concat_split/14_array_split/)

### [4.11 난수 생성](11_random/)
데이터 분석 시뮬레이션 및 무작위 샘플링을 위한 난수 발생 모듈(`np.random`) 기법을 익힙니다.
* [4.11.1 난수(Random) 생성기](11_random/01_intro_generator/)
* [4.11.2 실수(확률) 베이스](11_random/02_random_float/)
* [4.11.3 정수형 주사위 굴리기](11_random/03_random_integers/)
* [4.11.4 자연스러운 정규분포 난수](11_random/04_standard_normal/)

### [4.12 넘파이 종합 연습문제](12_exercises/)
지금까지 배운 배열 조작, 다차원 인덱싱, 불리언 필터링 등을 결합하여 파이썬 프로그래밍 실전 능력을 종합적으로 점검합니다.
* [4.12.1 기본기 점검](12_exercises/01_basic/)
* [4.12.2 데이터 필터링](12_exercises/02_indexing_boolean/)
* [4.12.3 축(Axis) 연산 응용](12_exercises/03_axis_operations/)
* [4.12.4 기초 시각화 연동](12_exercises/04_visualization/)
