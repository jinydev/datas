...
## 4.2.1 내장함수 array()로 ndarray 생성

**[비유] 파이썬 리스트 vs 넘파이 배열 (가방 vs 컨테이너 트럭)**
- 파이썬 리스트는 다양한 물건(숫자, 문자, 객체)을 막 섞어 담을 수 있는 **가방**이라면, 넘파이 배열은 오직 '규격화된 화물'만 가득 싣고 매우 빠르게 달리는 **컨테이너 트럭**입니다.

### ndarray의 핵심 원리
`ndarray`는 **N-Dimension Array(N차원 배열)**의 줄임말로, 파이썬 리스트와는 근본부터 다릅니다.
1. **동일한 데이터 타입 (Homogeneous)**: 리스트는 숫자와 문자를 섞을 수 있지만, `ndarray`는 오직 **한 종류**만 담아야 연산 속도가 극대화됩니다.
2. **연속된 메모리 배치 (Contiguous Memory)**: 데이터가 메모리에 빈틈없이 연속으로 붙어 있어, CPU가 순식간에 읽어들일 수 있습니다.
