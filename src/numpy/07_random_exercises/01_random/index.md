...
## 4.7.1 numpy의 난수와 다양한 함수

### 4.7.1 난수 생성

**[비유로 이해하기: 게임 속 강화 확률과 주사위 굴리기]**
게임에서 아이템 강화 성공이나 몬스터 드랍률 같은 "확률"은 필수 요소입니다. Numpy의 `random` 모듈이 이 역할을 완벽하게 수행합니다.
- **`rand()`**: 0~1 사이의 치우침 없는 실수 난수를 뽑습니다 (균등 분포).
- **`randn()`**: 평균 0 근처에 많이 몰려있는 실수 난수를 뽑습니다 (정규 분포).
- **`randint()`**: 특정 범위 안에서 정수형 주사위를 굴립니다.
