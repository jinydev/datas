...
# 4.2.10 초기화되지 않은 쓰레기값 배열을 생성하는 empty()
...
이럴 때 쓰는 극강의 속도 최적화 도구가 바로 **`np.empty()`**입니다. 이 함수는 내용을 무언가로 지우거나 채우는 시간을 전부 건너뛰고, 컴퓨터 메모리 어딘가에 남아있던 **'과거 삭제된 데이터의 파편(쓰레기값, Garbage Value)'**을 그대로 둔 채 빈 껍데기 상자판만 번개처럼 빠르게 던져줍니다.
