...
file_path = r'd:\site\jinydev\datas\src\python\01_basic\07_built_in_functions\index.md'
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

old_text = '''위의 경우 `year` 변수가 숫자 2005가 아닌 문자열 `"2005"`로 저장되어 있기 때문에 나눗셈이나 빼기 연산이 불가능합니다. 이를 해결하기 위해 파이썬 내장 형 변환 캐스팅 함수를 사용할 수 있습니다.
...
new_text = '''위의 경우 `year` 변수가 숫자 2005가 아닌 문자열 `"2005"`로 저장되어 있기 때문에 2026 - "2005" 같은 수학적 연산을 시도하면 즉각 에러가 발생합니다. 종이 상자("2005")와 쇳덩어리 숫자(2026)는 연산이 불가능하기 때문입니다.
...
if old_text in content:
    ...
    print("Exact old text not found.")
