---
layout: python
title: "3.3.9 내장 함수와 메서드 활용"
description: "본 장에서는 별도 패키지 설치 없이 파이썬이 기본 탑재하고 있는 강력하고 유용한 뼈대 수공구들인 내장 함수(Built-in Function)와, 각각의 데이터 공장(객체)이 지원하는 전용 메서드(Method)를 두루 익힙니다. 이 도구들을 능숙하게 다루는 것이야말로 진정한..."
---

# 3.3.9 내장 함수와 기본 메서드 활용
...
> [!NOTE] 
> **기본 내장 수학 함수(`abs`, `round`, `max` 등) vs `math` 모듈?**
> 일상적인 가벼운 계산(반올림, 최댓값)은 파이썬이 기본 제공하는 위 표의 내장 함수들로 충분합니다. 하지만 게임 물리 엔진을 짜거나 데이터의 로그(log), 삼각함수(sin, cos)를 다루는 전문 공학 계산을 하려면, 다음 장에서 배울 강력한 전용 공구함인 **`math` 모듈**을 수입(`import`)해 와야 합니다.
