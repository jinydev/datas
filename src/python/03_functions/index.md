...
description: "내가 짠 파이썬 코드가 수십 줄, 수백 줄 길어지기 시작하면 코드의 중복과 복잡함에 압도당하게 됩니다. 코드를 재사용 가능한 단위로 묶어서 블록화하는 핵심 개념인 '함수(Function)'에 대해 학습합니다. 본 장에서는 함수라는 단어가 가진 어원적/수학적 기원부터 시작하..."
---
...
### [3.3.3 함수 작동 흐름 시퀀스 다이어그램](./03_function_operation_flow/)
함수가 호출될 때 메인 프로그램의 메모리 제어권이 어떻게 함수 내부의 블랙박스 공간으로 일시 정지(Pause) 및 워프(Jump)하고, 연산 결과만을 들고 깔끔하게 귀환(Return)하는지 그 입체적인 생애주기를 시퀀스 다이어그램으로 한눈에 조망합니다.

### [3.3.4 함수 선언 문법 (def)과 호출(Call)](./04_function_usage/)
...
### [3.3.5 함수 호출과 제어 흐름의 도약](./05_function_call_flow/)
...
### [3.3.8 람다(Lambda) 익명 함수](./08_lambda_function/)
이름조차 짓기 귀찮은 일회용 함수인 `lambda`의 수학적 탄생 역사(알론조 처치)를 엿보고 코드를 극단적으로 얇게 썰어내는 방법을 익힙니다. 

### [3.3.9 마법의 거울, 재귀(Recursion)](./09_recursive_function/)
거울 속의 자신을 보듯 자기가 자기를 꼬리 물며 끝없이 호출하는 트리 탐색의 원조, **재귀 함수**의 마법 메커니즘을 훈련합니다.

### [3.3.10 (참고) 파이썬 내장 함수와 메서드](./10_built_in_functions/)
파이썬 생태계에 입장하자마자 언제든 뽑아 쓸 수 있도록 든든하게 제공되는 핵심 내장 함수(`len()`, `max()`, `sum()`, `enumerate()` 등)들의 활용법을 살펴봅니다. 코드의 길이를 드라마틱하게 줄여줄 치트키 창고입니다.

### [3.3.11 (심화) Python 파이썬과 수학](./11_math_module/)
