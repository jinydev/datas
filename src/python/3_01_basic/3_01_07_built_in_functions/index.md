...
### 실습예제
실습을 통하여 입출력과 데이터 형 변환(Casting)을 확인해 봅시다.
...
위의 경우 `year` 변수가 숫자 2005가 아닌 문자열 `"2005"`로 저장되어 있기 때문에 나눗셈이나 빼기 연산이 불가능합니다. 


이를 해결하기 위해 파이썬 내장 형 변환 캐스팅 함수를 사용할 수 있습니다.
...
파이썬은 객체지향 언어입니다. 따라서 모든 객체는 내부에 고유한 동작을 수행하는 **메서드(Method)**를 갖추고 있습니다. 객체에 점(`.`)을 찍어 호출하며 데이터의 변형과 추출을 이끕니다.
...
print(cleaned_text) # "HELLO PYTHON WORLD"
