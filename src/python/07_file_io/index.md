...
## 학습목표
본 장에서는 번거로운 패키지 설치 없이 파이썬 내장 스탠다드 기능만으로 하드디스크에 있는 텍스트 파일을 직접 열고, 읽고, 쓰는 가장 원초적인 제어 방법을 터득합니다. 특히 예기치 못한 에러 상황에서도 메모리 자원의 누수를 완벽히 차단하며 파일을 자동으로 닫아주는 **`with` 구문**의 가장 안전한 실무 패턴 구조를 몸에 익힙니다.
...
## 정리
향후 데이터 분석 과정에서 수 기가바이트의 엑셀이나 CSV를 다룰 때는 분명 Pandas 등을 쓰게 되겠지만, 로컬의 간단한 설정 파일이나 시스템의 로그를 한 줄씩 임시로 남겨 기록(Log)해야 할 땐 무거운 패키지 대신 빠르고 즉각적인 기본 파일 입출력(File I/O) 기능이 체감상 훨씬 많이 쓰입니다. `open()`을 불렀으면 반드시 `close()`가 따라와야 한다는 압박감에서 우리를 해방시켜 준 고마운 `with` 블록을 언제나 버릇처럼 애용합시다.
