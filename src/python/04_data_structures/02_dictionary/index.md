...
## 학습목표
본 장에서는 맹목적인 순서가 아닌 고유한 이름표(Key)를 달아 데이터를 짝지어 보관하는 **'딕셔너리(Dictionary)'**의 내부 작동 원리와 극대화된 검색 속도의 장점을 배웁니다. 데이터를 능숙하게 조작하는 기본기부터 시작해, 죽지 않는 단단하고 방어적인 코드를 작성하게 해주는 `get()` 탐색 메서드와 병합 스킬인 `update()`까지 체득합니다.
...
## 정리
데이터를 막연한 0, 1, 2 등의 인덱스 위치로 기억할 필요 없이 "나이", "회원아이디" 같은 명확한 의미 속성(Key)으로 바로 찾아 쓰는 사전의 무서운 직관성은 모던 네트워크 환경을 완전히 지배해 버린 JSON 통신 데이터 포맷과 일맥상통합니다. 특히 실존하지 않는 데이터를 참조하려다 파이썬 엔진이 멈춰버리는 불행을 미연에 방지해 주는 `.get()` 메서드는 꼭 버릇처럼 꺼내 쓰시길 권장합니다.
