...
## 학습목표
본 장에서는 중복된 코드 작성을 극한으로 줄여주는 **'상속(Inheritance)'**의 원리와 부모 클래스로부터 유전자를 물려받는 자식 클래스의 확장법을 다룹니다. 또한 부모의 행동을 입맛에 맞게 개조하는 오버라이딩(Overriding)과, 똑같은 지시를 내려도 객체마다 각자의 방식으로 다르게 수행하는 **'다형성(Polymorphism)'**의 쾌감을 맛봅니다.
...
## 정리
부모가 가진 수만 줄의 기능을 단 한 줄의 상속 선언(`class 펭귄(조류)`)만으로 그대로 꿀꺽 삼키는 상속의 위력은 객체지향의 가장 큰 축복입니다. 게다가 반복문 안에서 `동물.울어라()` 하나만 시켰을 뿐인데 개는 짖고 고양이는 야옹하는 다형성(Polymorphism)의 마법은, 여러분의 메인 제어 코드를 극도로 짧고 우아하게 만들어 주는 최강의 논리적 확장성 무기입니다.
