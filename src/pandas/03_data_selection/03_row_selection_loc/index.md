...
description: "데이터프레임(Matrix) 공간에서, 인덱스 축(Axis 0)에 부여된 사람이 읽을 수 있는 명시적 레이블(Label, 문자열 등)을 키형태로 사용하여 특정 행 벡터(Row Vector)들의 집합을 추출하는 과정입니다."
---
...
## 🪄 [실습 3] 여러 명의 데이터를 동시에 뽑기 (명단 넘기기)

계속해서 아래 코드를 추가합니다.

### 1단계: 명단을 이용한 여러 행 동시 추출
...
## 🪄 [실습 5] loc 안에 조건식(Boolean) 넣기

이번에는 `pandas_02.py` 파일을 생성해 봅니다. 앞서 만든 성적표 데이터(`pf`) 코드를 상단에 복사한 뒤 다음 실습을 진행합니다.
