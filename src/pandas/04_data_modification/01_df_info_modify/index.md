...
description: "대규모 행렬(데이터프레임)을 메모리에 적재한 직후 분석의 방향을 설정하기 위해 필수적으로 수행하는 단계입니다. 데이터의 구조적 메타데이터(차원수, 데이터 타입, 결측치 여부)를 파악하고, 각 열(Vector)에 대한 1차 기술 통계(Descriptive Statistics..."
---
...
> - [📥 df_info_modify_practice.ipynb 파일 다운로드](./df_info_modify_practice.ipynb) (클릭 또는 마우스 우클릭 후 '다른 이름으로 링크 저장')
...
![메타데이터와 통계량 연산 수학적 도식](./img/info_scan_math.svg)

## 🏷️ 비유로 이해하기: 환자의 기초 건강검진 리포트
...
![건강검진 리포트 비유 도식](./img/info_scan_analogy.svg)

---

## 🪄 [실습 1] 외부 서울시 교통사고 데이터 불러오기

VS Code나 주피터 노트북을 열고 `pandas_01.py` 파일을 생성하여 단계별로 실습을 진행합니다.

### 1단계: CSV 파일 읽기 및 데이터 확인
...
## 🪄 [실습 3] 데이터의 대략적인 크기와 수치 요약: `.describe()`

계속해서 아래 코드를 추가합니다.

### 1단계: 기초 통계량 요약 리포트 확인
...
## 🪄 [실습 4] 꿀팁: 행과 열 거꾸로 돌려보기 (Transpose)

새로운 실습을 위해 `pandas_02.py` 파일을 생성합니다. 앞서 작성한 데이터 불러오기 코드를 파일 상단에 복사한 뒤 실습을 진행합니다.
