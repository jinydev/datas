...
description: "단순 리스트를 딕셔너리로 묶어 데이터프레임을 만들면 0, 1, 2... 순서대로 단순 병합이 일어납니다. 하지만 각자의 고유한 주소 공간(Index)을 가진 Series들을 딕셔너리로 결합하면, 판다스는 내부적으로 합집합(Union) 연산을 수행하여 \"동일한 인덱스를 가진..."
---
...
> - [📥 df_from_series_dict_practice.ipynb 파일 다운로드](./df_from_series_dict_practice.ipynb) (클릭 또는 마우스 우클릭 후 '다른 이름으로 링크 저장')
...
![인덱스 합집합을 통한 정렬 연산](./img/df_series_dict_math.svg)

## 🏷️ 비유로 이해하기: 퍼즐 조각 맞춰 끼우기
...
![이름표를 기준으로 퍼즐을 끼워맞추는 비유](./img/df_series_dict_analogy.svg)
...
`pandas_03.py` 파일을 생성하고 마지막 실습을 진행합니다.

### 1단계: 기준이 되는 전체 명단(Index) 강제하기
