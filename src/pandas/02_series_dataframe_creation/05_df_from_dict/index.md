...
description: "수학에서 집합 $X$의 각 원소가 대응하는 종속 집합 $Y$를 가지는 형태를 프로그래밍에서는 Key: Value 쌍(Dict)으로 표현합니다. 딕셔너리를 데이터프레임으로 변환하는 과정은 각 열의 최상단 라벨(Column Header)을 Key로, 해당 열에 쭉 이어질 세로..."
---
...
앞선 실습 코드(`pandas_01.py`) 아래에 다음 코드를 추가하여 데이터프레임의 내부 구조를 확인해 봅니다.
...
위 데이터프레임의 속성을 다시 들여다보면, 이제 `index`도 숫자가 아닌 문자열 이름표로 바뀌어 있습니다. 작성한 코드 아래에 다음을 추가해 보세요.
