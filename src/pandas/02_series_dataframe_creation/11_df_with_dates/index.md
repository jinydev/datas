...
description: "$T$라는 시간 축(Time Axis) 위에 정사영된 관측치(Observations)들의 다차원 배열입니다. 단순한 라벨 문자열이나 고정된 정수가 아니라, 흐르는 시간(Datetime객체)을 인덱스 축으로 설정함으로써 주식 가격 예측, 기상 예측 등 시간에 따른 변화량을 ..."
---
...
> - [📥 df_with_dates_practice.ipynb 파일 다운로드](./df_with_dates_practice.ipynb) (클릭 또는 마우스 우클릭 후 '다른 이름으로 링크 저장')
...
![시간 축 인덱스가 결합된 다차원 매트릭스](./img/df_time_math.svg)

## 🏷️ 비유로 이해하기: 은행의 일일 거래 장부
...
![가장 왼쪽 첫 줄에 날짜가 박힌 은행 장부 비유](./img/df_time_analogy.svg)

---

## 🪄 [실습 1] 캘린더 생성기(date_range)로 날짜 축 준비하기
...
## 🪄 [실습 2] 난수(Random) 데이터와 결합하여 시계열 표 완성하기

작성한 코드 아래에 다음 코드를 추가하여 시계열 데이터프레임을 완성합니다.
