...
description: "데이터프레임의 각 열(Column)은 독립적인 벡터이므로 서로 다른 메모리 크기와 형태를 가지는 자료형(Data Type)을 수용할 수 있습니다. dtypes 속성은 각 컬럼 벡터가 C/C++ 레벨에서 어떤 바이트 크기(예: int64, float32, object)로 할..."
---
...
### 1단계: 초소형 자료형(int8)을 적용한 데이터프레임 생성
...
앞서 만든 `pandas_01.py` 코드로 돌아가, 그 아래에 다음 코드를 추가합니다.
