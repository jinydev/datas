...
description: "수학에서 등차수열(공차가 일정한 수열)을 만들듯, 시작점(start)과 끝점(end) 사이에서 일정한 시간 간격(freq)을 공차로 삼아 연속된 시간(Datetime) 벡터를 자동으로 생성해 주는 함수입니다. 시계열(Time-series) 분석을 할 때 필수적인 기준 축(..."
---
...
> - [📥 date_range_practice.ipynb 파일 다운로드](./date_range_practice.ipynb) (클릭 또는 마우스 우클릭 후 '다른 이름으로 링크 저장')
...
![일정한 시간 공차(Freq)를 갖는 등차수열 시퀀스](./img/date_range_math.svg)

## 🏷️ 비유로 이해하기: 회사 캘린더 자동 폭격기
...
![캘린더 자동 폭격기 비유](./img/date_range_analogy.svg)

---

## 🪄 [실습 1] 가장 기본적인 날짜 시퀀스 뽑기 (일별)

VS Code나 주피터 노트북을 열고 `pandas_01.py` 파일을 생성하여 단계별로 실습을 진행합니다.

### 1단계: 일별 기준 날짜 뽑기
기준 시작 날짜와 몇 개를 뽑을지(`periods`)만 지정하면, 기본적으로 '하루 단위(일별)'로 날짜를 생성해 `DatetimeIndex`라는 특별한 시계열 구조체로 반환합니다.
...
### 2단계: 끝나는 날짜 지정하기 (end)
개수(`periods`) 대신 종료일(`end`)을 줄 수도 있습니다. (종료일도 배열에 포함됩니다.) 작성한 코드 아래에 추가해 보세요.
...
## 🪄 [실습 3] "특정 요일", "월말/월초" 단위로 뽑아내기

`pandas_03.py` 파일을 생성하고 마지막 실습을 진행합니다.

### 1단계: 특정 요일만 뽑기 (예: 주간회의 일자)
...
### 2단계: 단위의 기준이 월말이냐 월초냐? (`M` vs `MS`)
- `M` (Month End): 월별 **마지막 날**을 기준으로 생성합니다.
- `MS` (Month Start): 월별 **1일**을 기준으로 생성합니다. 앞선 코드 아래에 다음을 추가하세요.
