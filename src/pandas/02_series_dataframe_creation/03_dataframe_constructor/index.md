...
description: "수학에서 특정 차원과 형태를 가진 행렬을 정의($A_{m \times n}$)하듯, 판다스에서 DataFrame 클래스 생성자는 $m$개의 행(Index)과 $n$개의 열(Columns)을 가지는 데이터를 메모리 공간에 어떻게 적재할지 규칙을 선언하는 초기화(Initial..."
---
...
## 🪄 [실습 1] 파라미터 조작해보기 (코드 실습)

VS Code나 주피터 노트북을 열고 `pandas_01.py` 파일을 생성하여 단계별로 실습을 진행합니다. 아무 옵션도 주지 않았을 때와, 우리가 직접 `index`와 `columns`를 지정했을 때의 차이를 비교해 봅니다.

### 1단계: 원시 데이터 및 기본 옵션 생성
아무 옵션 없이 기본값으로 데이터프레임을 생성해 봅니다.
```python
import pandas as pd
import numpy as np
...
### 2단계: 상세 옵션(index, columns, dtype) 지정하기
이번에는 직접 상세 주문서를 작성하여 표를 다듬어 봅니다. 앞선 코드 아래에 다음 코드를 추가하세요.
