...
기본 1차원 배열(Array)이 `x[0], x[1]`처럼 순차적 숫자 위치로만 데이터에 접근한다면, Pandas의 시리즈(Series)는 인덱스 도메인(Index)에서 값 코도메인(Value)으로 투사하는 일종의 **1대1 수치형 함수 기록지**와 같습니다.

**[비유로 이해하기: 이름표가 달린 포스트잇 묶음]**
수첩에 적어둔 전화번호부나 친구들의 성적표를 상상해 보세요. `[철수, 영희, 민수]` (Index)라는 이름표를 당기면 그 밑에 붙어 있는 `[80점, 95점, 100점]` (Values)이라는 값이 함께 딸려오는 구조입니다.

---

### [1단계] Series 생성: 다양한 재료로 만들기
