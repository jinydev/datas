...
file_path = r'd:\site\jinydev\datas\src\python\01_basic\07_math_module\index.md'

try:
    with open(file_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()
        
    # Replace lines 55-60 (0-indexed 54 to 59): ### 제곱근과 지수, 로그 함수 ...
    # We will just find the index of "### 제곱근과 지수, 로그 함수"
    ...
    for i, line in enumerate(lines):
        if "### 제곱근과 지수, 로그 함수" in line:
            idx_sqrt = i
        elif "### 천정과 바닥값, 내림 함수" in line:
            idx_ceil = i
        elif "print(m.trunc(3.1415))" in line:
            idx_trunc_end = i + 2 # include the closing ```
            
    if idx_sqrt != -1 and idx_ceil != -1 and idx_trunc_end != -1:
        # Assemble new content
        new_sqrt_section = """### 제곱근과 지수, 로그 함수

함수 `m.sqrt(4)` 호출은 4의 제곱근인 2를 반환합니다. 파이썬 자체에 거듭제곱(`**`) 기호가 이미 있지만, 전체적인 계산식의 결을 수학적으로 통일하기 위해 `math.pow(x, y)` 함수도 자주 씁니다.
...
"""
        ...
        new_ceil_section = """### 천장, 바닥, 그리고 무자비한 절단 (올림/내림/버림)
...
```
"""
        lines_before = lines[:idx_sqrt]
        lines_after = lines[idx_trunc_end:]
        
        final_text = "".join(lines_before) + new_sqrt_section + new_ceil_section + "".join(lines_after)
        
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(final_text)
            
        print("Success: Arrays matched and content replaced!")
    else:
        print(f"Failed to find indexes. Sqrt:{idx_sqrt}, Ceil:{idx_ceil}, Trunc:{idx_trunc_end}")
