...
// Use a very permissive regex to match everything from "위의 경우..." down to "...bool("False") --> True)."
const targetRegex = /위의 경우 `year` 변수가 숫자 2005가 아닌 문자열 `"2005"`로 저장되어 있기 때문에[\s\S]*?비어있지 않은 문자열은 단어에 상관없이 `True` 로 평가됩니다 \(`bool\("False"\) --> True`\)\./;
...
if (targetRegex.test(content)) {
    content = content.replace(targetRegex, new_text);
    fs.writeFileSync(file_path, content, 'utf-8');
    console.log("Replace successful via Master Regex!");
} else {
    console.log("Exact old text not found via Master Regex.");
}
